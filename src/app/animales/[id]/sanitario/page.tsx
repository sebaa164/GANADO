...
import { SanitarioView } from '@/components/sanitario/SanitarioView'

interface Props {
  params: Promise<{ id: string }>
}

export default async function AnimalSanitarioPage({ params }: Props) {
      ...
      <div className="w-full">
        ...
        <nav aria-label="Secciones del animal" className="flex gap-1 border-b border-gray-200 mb-6">
          ...
          <Link
            ...
            className="px-4 py-2 text-sm font-medium text-green-dark border-b-2 border-green-dark font-semibold"
          >
            Sanitario
          </Link>
        </nav>

        <SanitarioView animalId={id} />
      </div>
    </MainLayout>
  )
}
