...
## BE-2.2 — Motor de reglas FastAPI agregado

Se mantiene Laravel como parte del proyecto original y se agrega/completa el backend FastAPI en `backend/` para alinear el repositorio con el stack definido del proyecto.

La implementación BE-2.2 agrega motor de reglas para alertas automáticas, tabla `reglas_alerta`, worker Redis Pub/Sub, endpoint de evaluación de eventos y publicación de `alerta.creada`.

Documentación: `backend/docs/fase_2/BE-2.2_motor_reglas_alertas.md`.
